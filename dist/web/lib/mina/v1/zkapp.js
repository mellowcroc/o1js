...
import { Empty, analyzeMethod, compileProgram, computeMaxProofsVerified, sortMethodArguments, } from '../../proof-system/zkprogram.js';
...
import { AccountUpdate, AccountUpdateForest, AccountUpdateLayout, Authorization, Body, Events, Permissions, TokenId, ZkappCommand, ZkappPublicInput, zkAppProver, } from './account-update.js';
...
// external API
export { SmartContract, declareMethods, method };
        ...
        if (!methodIntfs.every((m) => m.methodName in methodMetadata) && !inAnalyze()) {
            ...
            try {
                for (let methodIntf of methodIntfs) {
                    let accountUpdate;
                    let { rows, digest, gates, summary, proofs } = await analyzeMethod(ZkappPublicInput, methodIntf, async (publicInput, publicKey, tokenId, ...args) => {
