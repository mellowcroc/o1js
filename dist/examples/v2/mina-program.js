        ...
        init: {
            ...
            async method(env, value) {
                return {
                    ...
                    setState: {
                        x: V2.Update.from(value, Field(0)),
                        address: V2.Update.from(env.accountId.publicKey, PublicKey.empty()),
                    },
                    pushActions: [
                        [Field(1), Field(2)],
                        [Field(3), Field(4)],
...
const authEnv = {
    networkId: 'testnet',
    async getPrivateKey(pk) {
        if (pk === feePayerAddress)
            return feePayerKey;
        if (pk === app.accountId.publicKey)
            return appKey;
        throw new Error();
    },
};
const testTransaction = await V2.createZkappCommand(testLedger, V2.LocalChain.initial(), authEnv, {
    feePayer: feePayerAddress,
    fee: V2.MinaAmount.fromMINA(1),
}, async (ctx) => {
    ctx.add(V2.AccountUpdate.create({
    ...
    await TestProgram.init(ctx, app.accountId, Field(3));
});
console.log(JSON.stringify(testTransaction, null, 2));
//# sourceMappingURL=mina-program.js.map