...
import { VotingApp } from './factory.js';
...
let voterStore = new OffchainStorage(3);
let candidateStore = new OffchainStorage(3);
let votesStore = new OffchainStorage(3);
...
tx = await Mina.transaction(feePayer, async () => {
    ...
    /*
      ...
      */
    0n, Member.from(PrivateKey.random().toPublicKey(), UInt64.from(150)), voterStore);
...
tx = await Mina.transaction(feePayer, async () => {
    ...
    /*
      ...
      but instead a public key
      */
    1n, Member.from(PrivateKey.random().toPublicKey(), UInt64.from(160)), voterStore);
    contracts.voting.voterRegistration(m);
});
await tx.prove();
await tx.sign([feePayer.key]).send();
...
tx = await Mina.transaction(feePayer, async () => {
    ...
    /*
      ...
      but instead a public key
      */
    2n, Member.from(PrivateKey.random().toPublicKey(), UInt64.from(170)), voterStore);
    contracts.voting.voterRegistration(m);
});
await tx.prove();
await tx.sign([feePayer.key]).send();
...
tx = await Mina.transaction(feePayer, async () => {
    ...
    /*
      ...
      but instead a public key
      */
    0n, Member.from(PrivateKey.random().toPublicKey(), UInt64.from(250)), candidateStore);
...
tx = await Mina.transaction(feePayer, async () => {
    ...
    /*
      ...
      but instead a public key
      */
    1n, Member.from(PrivateKey.random().toPublicKey(), UInt64.from(400)), candidateStore);
    contracts.voting.candidateRegistration(m);
});
await tx.prove();
await tx.sign([feePayer.key]).send();
...
/*
  ...
  */
console.log('still initial root? ', contracts.candidateContract.committedMembers.get().equals(initialRoot).toBoolean());
console.log('still initial root? ', contracts.voterContract.committedMembers.get().equals(initialRoot).toBoolean());
/*
...
console.log('candidate root? ', contracts.candidateContract.committedMembers.get().equals(candidateStore.getRoot()).toBoolean());
console.log('voter root? ', contracts.voterContract.committedMembers.get().equals(voterStore.getRoot()).toBoolean());
/*
...
console.log('1 vote sequence event? ', (await contracts.voting.reducer.fetchActions()).length === 1);
/*
    counting the votes
  */
tx = await Mina.transaction(feePayer, async () => {
...
await tx.sign([feePayer.key]).send();
// vote dispatches a new sequence events, so we should have one
console.log('votes roots equal? ', votesStore.getRoot().equals(contracts.voting.committedVotes.get()).toBoolean());
printResult();
function registerMember(i, m, store) {
