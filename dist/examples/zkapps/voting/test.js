import { Mina, AccountUpdate, Field, PrivateKey, UInt64, UInt32, Permissions } from 'o1js';
 ...
 */
export async function testSet(contracts, params, storage) {
    ...
    if (runTestingPhases[1]) {
      ...
      */
        ...
        let verificationKeySet = await deployContracts(contracts, params, Field(0), Field(0), Field(0), true);
        ...
        await assertValidTx(true, async () => {
        ...
        verificationKeySet.Local.addAccount(m.publicKey, m.balance.toString());
        await assertValidTx(false, async () => {
            await verificationKeySet.voting.voterRegistration(m);
        }, verificationKeySet.feePayer, 'Invalid proof');
    }
    if (runTestingPhases[2]) {
      ...
      */
        ...
        await assertValidTx(false, async () => {
            await permissionedSet.voting.voterRegistration(m);
        }, permissionedSet.feePayer, 'actions');
    }
    if (runTestingPhases[3]) {
      ...
      */
        ...
        let invalidSet = await deployInvalidContracts(contracts, params, votersStore.getRoot(), candidatesStore.getRoot(), votesStore.getRoot());
    ...
    if (runTestingPhases[4]) {
        ...
        let sequenceOverflowSet = await deployContracts(contracts, params, votersStore.getRoot(), candidatesStore.getRoot(), votesStore.getRoot());
        /*
        test case description:
      ...
      */
        ...
        if (actionsLength(sequenceOverflowSet.voterContract) < 3) {
            throw Error(`Did not emit expected actions! Only emitted ${actionsLength(sequenceOverflowSet.voterContract)}`);
        }
        try {
            let tx = await Mina.transaction(sequenceOverflowSet.feePayer.toPublicKey(), async () => {
    ...
    if (runTestingPhases[5]) {
        ...
        let { voterContract, candidateContract, voting, feePayer, Local } = await deployContracts(contracts, params, votersStore.getRoot(), candidatesStore.getRoot(), votesStore.getRoot());
        /*
        test case description:
      ...
      */
        ...
        console.log(`setting slot to ${params.electionPreconditions.startElection
            .sub(1)
        ...
        // register new member
        let newVoter1 = registerMember(0n, Member.from(PrivateKey.random().toPublicKey(), UInt64.from(15)), votersStore, Local);
        await assertValidTx(true, async () => {
        ...
        if (!initialAccumulatedMembers.equals(voterContract.accumulatedMembers.get()).toBoolean() ||
      ...
      */
        ...
        let newVoterLow = Member.from(PrivateKey.random().toPublicKey(), params.voterPreconditions.minMina.sub(1));
        addAccount(newVoterLow);
        await assertValidTx(false, async () => {
            await voting.voterRegistration(newVoterLow);
        }, feePayer, 'Balance not high enough!');
        console.log('attempting to register a voter with too high balance...');
        let newVoterHigh = Member.from(PrivateKey.random().toPublicKey(), params.voterPreconditions.maxMina.add(1));
        addAccount(newVoterHigh);
        await assertValidTx(false, async () => {
            await voting.voterRegistration(newVoterHigh);
        }, feePayer, 'Balance too high!');
        console.log('attempting to register the same voter twice...');
        await assertValidTx(false, async () => {
            await voting.voterRegistration(newVoter1);
        }, feePayer, 'Member already exists!');
        if (actionsLength(voterContract) !== 1) {
            throw Error('Should have emitted 1 event after registering only one valid voter');
        }
        /*
      ...
      */
        ...
        await assertValidTx(true, async () => {
            let newCandidate = registerMember(0n, Member.from(PrivateKey.random().toPublicKey(), params.candidatePreconditions.minMina.add(1)), candidatesStore, Local);
            // register new candidate
            await voting.candidateRegistration(newCandidate);
        }, feePayer);
        console.log('attempting to register another candidate...');
        await assertValidTx(true, async () => {
            let newCandidate = registerMember(1n, Member.from(PrivateKey.random().toPublicKey(), params.candidatePreconditions.minMina.add(1)), candidatesStore, Local);
            // register new candidate
            await voting.candidateRegistration(newCandidate);
        }, feePayer);
        let numberOfEvents = actionsLength(candidateContract);
        if (numberOfEvents !== 2) {
            throw Error(`Should have emitted 2 event after registering 2 candidates. ${numberOfEvents} emitted`);
        }
      ...
      */
        ...
        }, feePayer, 'Outside of election period!');
      ...
      */
        ...
        let fakeCandidate = Member.from(PrivateKey.random().toPublicKey(), params.candidatePreconditions.minMina.add(1));
        addAccount(fakeCandidate);
        await assertValidTx(false, async () => {
            // attempting to vote for the registered candidate
            await voting.vote(fakeCandidate, votersStore.get(0n));
        }, feePayer, 'Member is not a candidate!');
      ...
      */
        ...
        if (results[currentCandidate.publicKey.toBase58()] !== 1) {
            throw Error(`Candidate ${currentCandidate.publicKey.toBase58()} should have one vote, but has ${results[currentCandidate.publicKey.toBase58()]} `);
        }
      ...
      */
        ...
        let voter = Member.from(PrivateKey.random().toPublicKey(), params.voterPreconditions.minMina.add(1));
        addAccount(voter);
        await assertValidTx(false, async () => {
            await voting.voterRegistration(voter);
        }, feePayer, 'Outside of election period!');
        console.log('attempting to register candidate after election has ended');
        let candidate = Member.from(PrivateKey.random().toPublicKey(), params.candidatePreconditions.minMina.add(1));
...
function actionsLength(contract) {
