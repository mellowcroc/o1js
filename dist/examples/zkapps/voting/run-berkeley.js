import { AccountUpdate, Bool, fetchAccount, Mina, PrivateKey, PublicKey, Reducer, UInt32, UInt64, } from 'o1js';
...
let params = {
    candidatePreconditions: new ParticipantPreconditions(UInt64.from(0), UInt64.from(100_000_000_000)),
...
let storage = {
    votesStore: new OffchainStorage(3),
    candidatesStore: new OffchainStorage(3),
    votersStore: new OffchainStorage(3),
};
...
await tx.prove();
await (await tx.sign([feePayerKey, params.votingKey, params.candidateKey, params.voterKey]).send()).wait();
...
tx = await Mina.transaction({ sender: feePayerAddress, fee: 10_000_000, memo: 'Casting vote' }, async () => {
...
tx = await Mina.transaction({ sender: feePayerAddress, fee: 10_000_000, memo: 'Counting votes' }, async () => {
...
if (results[members[1].toBase58()] !== 1) {
    throw Error(`Candidate ${members[1].toBase58()} should have one vote, but has ${results[members[1].toBase58()]} `);
}
...
async function displayEvents(contract) {
    let events = await contract.fetchEvents();
    console.log(`events on ${contract.address.toBase58()}`, events.map((e) => {
...
//# sourceMappingURL=run-berkeley.js.map