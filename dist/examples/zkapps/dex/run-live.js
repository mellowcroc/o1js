...
if (successfulTransactions <= 1) {
    ...
    tx = await Mina.transaction(senderSpec, async () => {
        ...
        AccountUpdate.createSigned(sender).balance.subInPlace(Mina.getNetworkConstants().accountCreationFee.mul(3));
        await dex.deploy();
...
if (keyPairReleaseMessage)
    console.info(keyPairReleaseMessage);
async function ensureFundedAccount(privateKeyBase58) {
    ...
    if (balance === undefined || balance <= 15000000000n) {
        await Mina.faucet(sender);
        await sleep(1);
    }
    return { senderKey, sender };
}
function logPendingTransaction(pendingTx) {
    if (pendingTx.status === 'rejected')
...
async function getTokenBalances() {
    ...
    let user = 'user';
    try {
        balances.user.MINA = Mina.getBalance(addresses[user]).toBigInt() / 1000000000n;
    }
    catch { }
...
//# sourceMappingURL=run-live.js.map