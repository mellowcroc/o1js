...
async function atomicActionsTest({ withVesting }) {
    ...
    await tx.prove();
    await expect(tx.sign([feePayer.key, keys.dex]).send()).rejects.toThrow(/Cannot update field 'delegate'/);
     ...
     */
    console.log('changing permission to impossible and then trying to change delegate field - in one transaction');
    tx = await Mina.transaction(feePayer, async () => {
...
async function upgradeabilityTests({ withVesting }) {
    ...
    let options = withVesting ? { lockedLiquiditySlots: 2 } : undefined;
    let { Dex, DexTokenHolder, ModifiedDexTokenHolder, ModifiedDex, getTokenBalances } = createDex(options);
    // analyze methods for quick error feedback
    await DexTokenHolder.analyzeMethods();
    await Dex.analyzeMethods();
    // compile & deploy all zkApps
    ...
    console.log('deploying an upgraded DexTokenHolder contract (adjusted swap method) and Dex contract');
    ...
    expect(Mina.getAccount(addresses.dex, tokenX.deriveTokenId()).zkapp?.verificationKey?.data).toEqual(ModifiedDexTokenHolder._verificationKey?.data);
    expect(Mina.getAccount(addresses.dex, tokenY.deriveTokenId()).zkapp?.verificationKey?.data).toEqual(ModifiedDexTokenHolder._verificationKey?.data);
    ...
    expect(oldBalances.user.Y).toEqual(1000000n);
    expect(balances.user.Y).toEqual(1000024n);
    /**
    ...
    await tx.prove();
    await expect(tx.sign([feePayer.key, keys.dex]).send()).rejects.toThrow(/Cannot update field 'verificationKey'/);
...
//# sourceMappingURL=upgradability.js.map