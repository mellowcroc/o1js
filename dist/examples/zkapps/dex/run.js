...
async function main({ withVesting }) {
    ...
    expect(balances.user.Y).toEqual(oldBalances.user.Y - (USER_DX * oldBalances.dex.Y) / oldBalances.dex.X);
    ...
    if (!withVesting) {
        ...
        expect(balances.user.lqXY).toEqual(oldBalances.user.lqXY + (USER_DX * oldBalances.total.lqXY) / oldBalances.dex.X);
    }
    else {
    ...
    await tx.sign([keys.user]).send();
    [oldBalances, balances] = [balances, getTokenBalances()];
    console.log('redeem liquidity with both users in one tx (fails because of conflicting balance preconditions)');
    tx = await Mina.transaction(addresses.user2, async () => {
        AccountUpdate.createSigned(addresses.user2).balance.subInPlace(Mina.getNetworkConstants().accountCreationFee.mul(2));
        await dex.redeemLiquidity(UInt64.from(USER_DL));
        await dex.redeemLiquidity(UInt64.from(USER_DL));
    });
    await tx.prove();
    ...
    expect(balances.dex.X * balances.dex.Y).toBeGreaterThanOrEqual(oldBalances.dex.X * oldBalances.dex.Y);
    DexProfiler.stop().store();
}
