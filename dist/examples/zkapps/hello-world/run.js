...
console.log(`Attempting to update state from ${currentState} to 16 with incorrect Admin Private Key ...`);
...
try {
    console.log(`Attempting to update state from ${currentState} to the value that fails precondition of 30 ...`);
    txn = await Mina.transaction(feePayer1, async () => {
...
correctlyFails = false;
try {
    console.log(`Attempting to simultaneously update the current state of ${currentState} from multiple users ...`);
...
function handleError(error, errorMessage) {
    ...
    if (error.message.includes(errorMessage)) {
        correctlyFails = true;
        console.log(`Update correctly rejected with failing precondition. Current state is still ${currentState}.`);
    }
    else {
        throw Error(error);
    }
