...
await Provable.runAndCheck(() => {
     ...
     */
    assert(accountUpdateWitness.body.callDepth === 5, 'when witness block is executed, witness() recreates auxiliary parts of provable type');
    Provable.assertEqual(PrivateKey, accountUpdateWitness.lazyAuthorization.privateKey, accountUpdate.lazyAuthorization.privateKey);
});
/**
...
let result = await Provable.constraintSystem(() => {
     ...
     */
    let accountUpdateWitness = Provable.witness(AccountUpdate, () => {
     ...
     */
    assert(accountUpdateWitness.body.callDepth === 0, 'when witness block is not executed, witness() returns dummy data');
    Provable.assertEqual(AccountUpdate, accountUpdateWitness, accountUpdate);
});
/**
 ...
 */
console.log(`witnessing an account update and comparing it to another one creates ${result.rows} rows`);
/**
...
result = await Provable.constraintSystem(async () => {
    let { accountUpdate: accountUpdateWitness } = await AccountUpdate.witness(Empty, async () => ({ accountUpdate, result: undefined }), { skipCheck: true });
    Provable.assertEqual(AccountUpdate, accountUpdateWitness, accountUpdate);
});
console.log(`without all the checks on subfields, witnessing and comparing only creates ${result.rows} rows`);
/**
...
//# sourceMappingURL=advanced-provable-types.js.map