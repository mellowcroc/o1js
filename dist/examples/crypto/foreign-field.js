...
import { Provable, Scalar, SmartContract, State, createForeignField, method, state, } from 'o1js';
...
assert(z.mul === undefined); // z.mul() is not defined
assert(z.inv === undefined);
assert(z.div === undefined);
// to do multiplication, you need "almost reduced" fields:
let y = z.assertAlmostReduced(); // adds constraints to prove that z is, in fact, reduced
...
let constant = SmallField.from(1);
assert(constant instanceof SmallField.Canonical);
SmallField.from(10000n); // works because `from()` takes the input mod p
SmallField.from(-1); // works because `from()` takes the input mod p
...
__decorate([
    state(AlmostSmallField),
    __metadata("design:type", Object)
], MyContract.prototype, "x", void 0);
__decorate([
    method,
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [AlmostSmallField]),
...
let Fseqp256k1 = createForeignField((1n << 256n) - (1n << 32n) - 977n);
