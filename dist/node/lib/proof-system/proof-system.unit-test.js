...
import { CompiledTag, Empty, Void, ZkProgram, picklesRuleFromFunction, sortMethodArguments, } from './zkprogram.js';
...
it('pickles rule creation', async () => {
    ...
    function main(proof, shouldVerify) {
    ...
    // create pickles rule
    let rule = picklesRuleFromFunction(Empty, Field, main, { name: 'mock' }, methodIntf, [], [EmptyProof]);
    await equivalentAsync({ from: [field, bool], to: record({ field, bool }) }, { runs: 5 })((field, bool) => ({ field, bool }), async (field, bool) => {
        ...
        await Provable.runAndCheck(async () => {
            ...
            // call pickles rule
            let { publicOutput: [, publicOutput], shouldVerify: [, shouldVerify], } = await rule.main([0]);
...
it('pickles rule creation: nested proof', async () => {
    function main([first, _second]) {
        // first proof should verify, second should not
        first.proof.verify();
        // deep type inference
        first.proof.publicInput;
        first.proof.publicOutput;
    }
    // collect method interface
    ...
    CompiledTag.store(EmptyProgram, 'mock tag');
    // create pickles rule
    let rule = picklesRuleFromFunction(Empty, Void, main, { name: 'mock' }, methodIntf, [], [EmptyProof, EmptyProof]);
    ...
    await Provable.runAndCheck(async () => {
        ...
        // call pickles rule
        let { shouldVerify: [, shouldVerify1, shouldVerify2], previousStatements: [, ...previousStatements], } = await rule.main([0]);
...
expect(emptyMethodsMetadata.run).toEqual(expect.objectContaining({
    rows: 0,
...
const CounterProgram = ZkProgram({
    ...
    methods: {
        increment: {
            ...
            async method({ current, updated }, incrementBy) {
...
//# sourceMappingURL=proof-system.unit-test.js.map