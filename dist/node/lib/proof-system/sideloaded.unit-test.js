...
class SampleSideloadedProof extends DynamicProof {
}
SampleSideloadedProof.publicInputType = Field;
SampleSideloadedProof.publicOutputType = Void;
SampleSideloadedProof.maxProofsVerified = 0;
class SampleSideloadedProof2 extends DynamicProof {
}
SampleSideloadedProof2.publicInputType = Program2Struct;
SampleSideloadedProof2.publicOutputType = Field;
SampleSideloadedProof2.maxProofsVerified = 0;
const sideloadedProgram = ZkProgram({
    ...
    methods: {
        recurseOneSideloaded: {
            ...
            async method(publicInput, proof, vk) {
                proof.verify(vk);
        ...
        recurseTwoSideloaded: {
            ...
            async method(publicInput, proof1, vk1, proof2, vk2) {
                proof1.verify(vk1);
                proof2.verify(vk2);
                proof1.publicInput
...
__decorate([
    ...
    __metadata("design:paramtypes", [Field, SampleSideloadedProof, VerificationKey]),
    __metadata("design:returntype", Promise)
], SideloadedSmartContract.prototype, "setValue", null);
describe('sideloaded', async () => {
    ...
    const { proof: program2Proof } = await program2.foo({ field1: Field(1), field2: Field(2) }, Field(3));
    ...
    it('recurse one proof with zkprogram', async () => {
        const proof = SampleSideloadedProof.fromProof(program1Proof);
        const { proof: finalProof } = await sideloadedProgram.recurseOneSideloaded(Field(1), proof, program1Vk);
    ...
    it('recurse two different proofs with zkprogram', async () => {
        ...
        const finalProof = await sideloadedProgram.recurseTwoSideloaded(Field(4), proof1, program1Vk, proof2, program2Vk);
        expect(finalProof).toBeDefined();
    });
    ...
    it('should work if SL Proof classes are used in different ZkPrograms', async () => {
        ...
        const finalProof = await sideloadedProgram2.recurseTwoSideloaded(Field(5), proof1, program1Vk, proof2, program2Vk);
        expect(finalProof).toBeDefined();
    });
...
//# sourceMappingURL=sideloaded.unit-test.js.map