...
const FILE_PATH = path.isAbsolute(argv.file ?? '')
    ? argv.file
    : path.join(process.cwd(), argv.file ? argv.file : './tests/perf-regression/perf-regression.json');
 ...
 */
function createPerformanceSession(programName, methodsSummary, log = true) {
    const perfStack = [];
    ...
    return {
         ...
         */
            ...
            if (shouldLog && label) {
                console.log(`${label} ${programName ?? ''}${(label === 'prove' || label === 'verify') && methodName ? '.' + methodName : ''}... ${time.toFixed(3)} sec`);
            }
            // If neither --dump nor --check, we’re done.
            if (!DUMP && !CHECK)
                return;
            // Only act for compile/prove/verify with required context
            if (!programName || (label !== 'compile' && label !== 'prove' && label !== 'verify'))
                return;
            // Load the baseline JSON used for both DUMP and CHECK modes.
            const raw = fs.readFileSync(FILE_PATH, 'utf8');
            const perfRegressionJson = JSON.parse(raw);
            // --- compile ---
            if (label === 'compile') {
                if (DUMP) {
            ...
            if (label === 'prove' || label === 'verify') {
                ...
                if (CHECK) {
                    if (label === 'prove') {
                        checkMethodTime(perfRegressionJson, programName, methodName, info.digest, time, 'prove');
                    }
                    else {
                        checkMethodTime(perfRegressionJson, programName, methodName, info.digest, time, 'verify');
                    }
                    return;
                }
            }
...
const Performance = {
     ...
     */
    create(programName, methodsSummary, log) {
        return createPerformanceSession(programName, methodsSummary, log);
    },
};
/// HELPERS (dump/check)
function dumpCompile(perfRegressionJson, programName, time) {
...
function dumpMethodTime(perfRegressionJson, programName, methodName, info, time, label) {
    const prev = perfRegressionJson[programName];
    const merged = prev
...
function checkCompile(perfRegressionJson, programName, actualTime) {
...
function checkMethodTime(perfRegressionJson, programName, methodName, digest, actualTime, label) {
    checkAgainstBaseline({
        perfRegressionJson,
        programName,
        label,
...
// -------------------------
function validateMethodContext(label, cs, methodName, programName) {
    if (!cs || typeof cs !== 'object') {
        throw new Error(`methodsSummary is required for this label: ${label}. Pass it to Performance.create(programName, methodsSummary).`);
    }
    if (!methodName) {
    ...
    if (!info) {
        ...
        throw new Error(`The method "${methodName}" does not exist in the analyzed constraint system for "${programName}". ` +
...
function checkAgainstBaseline(params) {
    ...
    if (label === 'compile') {
        ...
        if (expected == null) {
            throw new Error(`No baseline compileTime for "${programName}". Run --dump (compile) to set it.`);
        }
        ...
        if (failed) {
            throw new Error(`Compile regression for ${programName}\n` +
    ...
    if (!baseMethod) {
        throw new Error(`No baseline method entry for ${programName}.${methodName}. Run --dump (${label}) to add it.`);
    }
    if (baseMethod.digest !== digest) {
        throw new Error(`Digest mismatch for ${programName}.${methodName}\n` +
            `  Actual:   ${digest}\n` +
            `  Expected: ${baseMethod.digest}\n`);
    }
    const expected = label === 'prove' ? baseMethod.proveTime : baseMethod.verifyTime;
    if (expected == null) {
        throw new Error(`No baseline ${label}Time for ${programName}.${methodName}. Run --dump (${label}) to set it.`);
    }
    const tol = expected < 0.2 ? timeTolSmall : timeTolDefault;
    const allowedPct = (tol - 1) * 100;
    const regressionPct = expected === 0 ? 0 : ((actualTime - expected) / expected) * 100;
    const failed = actualTime > expected * tol;
    logPerf(`${programName}.${methodName}`, label, expected, actualTime, regressionPct, allowedPct, failed);
    if (failed) {
        throw new Error(`${labelPretty} regression for ${programName}.${methodName}\n` +
            `  Actual:     ${actualTime.toFixed(3)}s\n` +
            `  Baseline:   ${expected.toFixed(3)}s\n` +
            `  Regression: +${Number.isFinite(regressionPct) ? regressionPct.toFixed(2) : '∞'}% (allowed +${allowedPct.toFixed(0)}%)`);
    }
}
function logPerf(scope, label, expected, actual, regressionPct, allowedPct, failed) {
    const COLORS = {
        reset: '\x1b[0m',
        red: '\x1b[31m',
        green: '\x1b[32m',
    ...
    if (failed)
        ...
        color = COLORS.green;
    console.log(`${COLORS.cyan}[Perf][${scope}]${COLORS.reset} ${label}: ` +
        `baseline=${expected.toFixed(6)}s, actual=${actual.toFixed(6)}s, ` +
        `${color}regression=${regressionPct >= 0 ? '+' : ''}${Number.isFinite(regressionPct) ? regressionPct.toFixed(2) : '∞'}%${COLORS.reset} ` +
