...
equivalentProvable({ from: [scalar], to: unit, verbose: true })(() => true, (s) => {
...
equivalentProvable({ from: [nonCanonicalScalar], to: unit, verbose: true })(() => false, (s) => {
    let sCanonical = Scalar.toCanonical(s);
    // equivalent to the input according to Provable.equal()
    Provable.equal(Scalar, s, sCanonical).assertTrue();
...
// helpers
function run(g1, g2, f, assert) {
    let result_out_circuit = f(g1, g2);
    runAndCheckSync(() => {
        let result_in_circuit = f(Provable.witness(Group, () => g1), Provable.witness(Group, () => g2));
        Provable.asProver(() => {
            assert(result_out_circuit.equals(result_in_circuit).toBoolean(), `Result for x does not match. g1: ${JSON.stringify(g1)}, g2: ${JSON.stringify(g2)}`);
        });
    });
}
function runScale(g, s, f, assert) {
    let result_out_circuit = f(g, s);
    runAndCheckSync(() => {
        let result_in_circuit = f(Provable.witness(Group, () => g), Provable.witness(s.constructor, () => s));
        Provable.asProver(() => {
            assert(result_out_circuit.equals(result_in_circuit).toBoolean(), `Result for x does not match. g: ${JSON.stringify(g)}, s: ${JSON.stringify(s)}
...
//# sourceMappingURL=group.unit-test.js.map