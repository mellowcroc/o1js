...
function expectCollision(hashA, hashB, msgA, msgB) {
    function formatPoseidonInput(values) {
        if (values.length === 0)
            return '[]';
...
function expectNoCollision(hashA, hashB, msgA, msgB) {
    function formatPoseidonInput(values) {
        if (values.length === 0)
            return '[]';
        return `[${values.map((x) => x.toString()).join(', ')}]`;
...
function expectEqualCircuit({ nameA, vkA, circuitA }, { nameB, vkB, circuitB }) {
    try {
...
function expectUnequalCircuit({ nameA, vkA }, { nameB, vkB }) {
    try {
        assert(vkA.verificationKey.data !== vkB.verificationKey.data);
    }
    catch (e) {
...
async function expectNoForgery(proofA, vkB) {
    try {
