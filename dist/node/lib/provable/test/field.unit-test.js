...
import { equivalentProvable as equivalent, oneOf, field, bigintField, throwError, unit, bool, } from '../../testing/equivalent.js';
...
equivalent1((x) => Fp.inverse(x) ?? throwError('division by 0'), (x) => x.inv());
equivalent2((x, y) => Fp.div(x, y) ?? throwError('division by 0'), (x, y) => x.div(y));
equivalent1(Fp.square, (x) => x.square());
equivalent1((x) => Fp.sqrt(x) ?? throwError('no sqrt'), (x) => x.sqrt());
equivalent({ from: [field, fieldOrBigint], to: bool })((x, y) => x === y, (x, y) => x.equals(y));
equivalent({ from: [field, fieldOrBigint], to: bool })((x, y) => x < y, (x, y) => x.lessThan(y));
equivalent({ from: [field, fieldOrBigint], to: bool })((x, y) => x <= y, (x, y) => x.lessThanOrEqual(y));
equivalent({ from: [field, fieldOrBigint], to: unit })((x, y) => x === y || throwError('not equal'), (x, y) => x.assertEquals(y));
equivalent({ from: [field, fieldOrBigint], to: unit })((x, y) => x !== y || throwError('equal'), (x, y) => x.assertNotEquals(y));
equivalent({ from: [field, fieldOrBigint], to: unit })((x, y) => x < y || throwError('not less than'), (x, y) => x.assertLessThan(y));
equivalent({ from: [field, fieldOrBigint], to: unit })((x, y) => x <= y || throwError('not less than or equal'), (x, y) => x.assertLessThanOrEqual(y));
equivalent({ from: [field], to: bool })((x) => {
    assert(x === 0n || x === 1n, 'not boolean');
    return x === 1n;
}, (x) => x.assertBool());
equivalent({ from: [field], to: unit })((x) => x === 0n || x === 1n || throwError('not boolean'), (x) => {
    let y = Provable.witness(Field, () => x.div(2));
    y.mul(2).assertBool();
});
equivalent({ from: [field], to: bool })((x) => (x & 1n) === 0n, (x) => x.isEven());
...
//# sourceMappingURL=field.unit-test.js.map