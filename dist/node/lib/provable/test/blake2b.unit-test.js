...
sample(Random.nat(400), 5).forEach((preimageLength) => {
    ...
    equivalentProvable({ from: [inputBytes], to: outputBytes, verbose: true })((x) => nobleBlake2b(x), (x) => Gadgets.BLAKE2B.hash(x), `blake2b preimage length ${preimageLength}`);
});
for (let { digest_length, preimage, hash } of testVectors()) {
    ...
    equivalentProvable({ from: [inputBytes], to: outputBytes, verbose: true })(() => Bytes.fromHex(hash).toBytes(), () => Gadgets.BLAKE2B.hash(Bytes.fromString(preimage), digest_length), `provable: blake2b preimage length ${preimage.length}`);
}
...
await equivalentAsync({
    from: [bytes(192)],
    to: bytes(64),
}, { runs: RUNS })(nobleBlake2b, async (x) => {
...
//# sourceMappingURL=blake2b.unit-test.js.map