...
for (let Curve of curves) {
    ...
    equivalentProvable({ from: [unequalPair], to: point, verbose: true })(([p, q]) => Curve.add(p, q), ([p, q]) => EllipticCurve.add(p, q, Curve), `${Curve.name} add`);
    equivalentProvable({ from: [point], to: point, verbose: true })(Curve.double, (p) => EllipticCurve.double(p, Curve), `${Curve.name} double`);
    equivalentProvable({ from: [point], to: point, verbose: true })(Curve.negate, (p) => EllipticCurve.negate(p, Curve), `${Curve.name} negate`);
    equivalentProvable({ from: [point], to: unit, verbose: true })((p) => Curve.isOnCurve(p) || throwError('expect on curve'), (p) => EllipticCurve.assertOnCurve(p, Curve), `${Curve.name} on curve`);
    equivalentProvable({ from: [point, scalar], to: point, verbose: true })((p, s) => {
        let sp = Curve.scale(p, s);
        assert(!sp.infinity, 'expect nonzero');
        return sp;
    }, (p, s) => EllipticCurve.scale(s, p, Curve), `${Curve.name} scale`);
}
//# sourceMappingURL=elliptic-curve.unit-test.js.map