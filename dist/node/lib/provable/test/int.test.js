...
describe('int', () => {
    ...
    describe('Int64', () => {
        ...
        describe('add', () => {
            ...
            it('5000+(-4000)=1000', () => {
                expect(Int64.from(5000)
                    .add(Int64.fromField(Field(-4000)))
                    .toString()).toEqual('1000');
            });
            it('(MAXINT/2+MAXINT/2) adds to MAXINT', () => {
                const value = ((1n << 64n) - 2n) / 2n;
                expect(Int64.from(value).add(Int64.from(value)).add(Int64.one).toString()).toEqual(UInt64.MAXINT().toString());
            });
        ...
        describe('sub', () => {
            ...
            it('(0-MAXINT) subs to -MAXINT', () => {
                expect(Int64.zero.sub(UInt64.MAXINT()).toString()).toEqual('-' + UInt64.MAXINT().toString());
            });
        });
        describe('toFields', () => {
        ...
        describe('fromFields', () => {
            ...
            it('fromFields(2^53-1) should be the same as Field(2^53-1)', () => {
                expect(Int64.fromFields([Field(String(NUMBERMAX)), Field(1)])).toEqual(Int64.from(NUMBERMAX));
            });
        });
    ...
    describe('UInt64', () => {
        describe('Inside circuit', () => {
            describe('add', () => {
                ...
                it('should throw on overflow addition', async () => {
                    await expect(Provable.runAndCheck(() => {
            ...
            describe('sub', () => {
                ...
                it('should throw on sub if results in negative number', async () => {
                    await expect(Provable.runAndCheck(async () => {
        ...
        describe('Outside of circuit', () => {
            describe('add', () => {
                ...
                it('(MAXINT/2+MAXINT/2) adds to MAXINT', () => {
                    const value = ((1n << 64n) - 2n) / 2n;
                    expect(new UInt64(value).add(new UInt64(value)).add(new UInt64(1)).toString()).toEqual(UInt64.MAXINT().toString());
                });
                it('should throw on overflow addition', () => {
                    expect(() => {
                        UInt64.MAXINT().add(1);
            ...
            describe('toBits() and fromBits()', () => {
                ...
                it('fails if we ask for fewer bits than needed', () => {
                    const len = 16;
                    const val = UInt64.from(0xffffffffn);
                    expect(() => val.toBits(len)).toThrow(`UInt64.toBits(): ${val.toString()} does not fit in ${len} bits`);
                });
                it('fails if we provide a bits array that is too long for UInt64', () => {
                    const len = 65;
                    const badBits = Array(len).fill(Bool(true));
                    expect(() => UInt64.fromBits(badBits)).toThrow(`UInt64.fromBits(): bit length must be 64 or less, got ${len}`);
                });
            });
        });
    });
        ...
        describe('Outside of circuit', () => {
            describe('add', () => {
                ...
                it('(MAXINT/2+MAXINT/2) adds to MAXINT', () => {
                    ...
                    expect(new UInt32(value).add(new UInt32(value)).add(new UInt32(1)).toString()).toEqual(UInt32.MAXINT().toString());
                });
                it('should throw on overflow addition', () => {
                    expect(() => {
                        UInt32.MAXINT().add(1);
            ...
            describe('toBits() and fromBits()', () => {
                ...
                it('fails if we ask for fewer bits than the value needs', () => {
                    ...
                    expect(() => val.toBits(len)).toThrow(`UInt32.toBits(): ${val.toString()} does not fit in ${len} bits`);
                });
                it('fails if we provide a bits array that is longer than 32 in fromBits()', () => {
                    const len = 33;
                    const badBits = Array(len).fill(Bool(true));
                    expect(() => UInt32.fromBits(badBits)).toThrow(`UInt32.fromBits(): bit length must be 32 or less, got ${len}`);
                });
            });
        });
    });
        ...
        describe('Outside of circuit', () => {
            describe('add', () => {
                ...
                it('(MAXINT/2+MAXINT/2) adds to MAXINT', () => {
                    ...
                    expect(new UInt8(value).add(new UInt8(value)).add(new UInt8(1)).toString()).toEqual(UInt8.MAXINT().toString());
                });
                it('should throw on overflow addition', () => {
                    expect(() => {
                        UInt8.MAXINT().add(1);
            ...
            describe('toBits() and fromBits()', () => {
                ...
                it('fails if we ask for fewer bits than the value needs', () => {
                    ...
                    expect(() => val.toBits(len)).toThrow(`UInt8.toBits(): ${val.toString()} does not fit in ${len} bits`);
                });
                it('fails if we provide a bits array that is longer than 8 in fromBits()', () => {
                    const len = 10;
                    const badBits = Array(len).fill(Bool(true));
                    expect(() => UInt8.fromBits(badBits)).toThrow(`UInt8.fromBits(): bit length must be 8 or less, got ${len}`);
                });
            });
        });
    });
