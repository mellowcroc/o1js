...
import { Ecdsa, EllipticCurve, Point, initialAggregator, verifyEcdsaConstant, } from '../gadgets/elliptic-curve.js';
...
import { bool, equivalentProvable, fromRandom, map, oneOf, record, } from '../../testing/equivalent.js';
...
{
    // generate a key pair
    let { privateKey, publicKey } = await crypto.subtle.generateKey({ name: 'ECDSA', namedCurve: 'P-256' }, true, ['sign']);
...
for (let Curve of curves) {
    ...
    const verify = (sig, noGlv) => {
        ...
        if (noGlv)
            Curve.hasEndomorphism = false; // hack to force non-GLV version
        try {
    ...
    const checkInputs = ({ signature: { r, s }, publicKey }) => {
    ...
    // positive test
    equivalentProvable({ from: [signature, noGlv], to: bool, verbose: true })(() => true, verify, `${Curve.name}: verifies`);
    // negative test
    equivalentProvable({ from: [badSignature, noGlv], to: bool, verbose: true })((s) => checkInputs(s) && false, verify, `${Curve.name}: fails`);
...
let signature = Ecdsa.Signature.fromHex('0x82de9950cc5aac0dca7210cb4b77320ac9e844717d39b1781e9d941d920a12061da497b3c134f50b2fce514d66e20c5e43f9615f097395a5527041d14860a52f1b');
...
function fromBase64Url(b64url) {
    let b64 = b64url.replace(/-/g, '+').replace(/_/g, '/') + '===='.slice(0, (4 - (b64url.length % 4)) % 4);
    return Uint8Array.from(atob(b64), (c) => c.charCodeAt(0));
}
//# sourceMappingURL=ecdsa.unit-test.js.map