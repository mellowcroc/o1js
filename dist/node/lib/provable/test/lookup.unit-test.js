...
// Range-check tests
{
    ...
    await equivalentAsync({ from: [uint(12), uint(12), uint(12)], to: boolean }, { runs: 3 })((x, y, z) => {
        assert(x < 1n << 12n);
        assert(y < 1n << 12n);
        assert(z < 1n << 12n);
        return true;
...
// Unit test with constant indices using the deprecated API for runtime tables
{
    ...
    await equivalentAsync({ from: [uint(12), uint(12), uint(12)], to: boolean }, { runs: 1 })((x, y, z) => {
        assert(x < 1n << 12n);
        assert(y < 1n << 12n);
        assert(z < 1n << 12n);
        return true;
...
//# sourceMappingURL=lookup.unit-test.js.map