...
import { array, equivalent, equivalentAsync, equivalentProvable, fromRandom, record, unit, } from '../../testing/equivalent.js';
import { Random } from '../../testing/random.js';
import { Field3, Gadgets } from '../gadgets/gadgets.js';
import { ZkProgram } from '../../proof-system/zkprogram.js';
import { Provable } from '../provable.js';
import { assert } from '../gadgets/common.js';
import { allConstant, and, constraintSystem, contains, equals, ifNotAllConstant, not, or, repeat, withoutGenerics, } from '../../testing/constraint-system.js';
...
let sign = fromRandom(Random.oneOf(1n, -1n));
let fields = [
    exampleFields.small,
...
for (let F of fields) {
    ...
    equivalentProvable({ from: [f], to: f })((x) => F.inverse(x) ?? throwError('no inverse'), (x) => ForeignField.inv(x, F.modulus), 'inv');
    eq2((x, y) => F.div(x, y) ?? throwError('no inverse'), (x, y) => ForeignField.div(x, y, F.modulus), 'div');
    equivalentProvable({ from: [f, f], to: unit })((x, y) => assertMulExampleNaive(Field3.from(x), Field3.from(y), F.modulus), (x, y) => assertMulExample(x, y, F.modulus), 'assertMul');
    // test for assertMul which mostly tests the negative case because for random inputs, we expect
    // (x - y) * z != a + b
    equivalentProvable({ from: [f, f, f, f, f], to: unit })((x, y, z, a, b) => assert(F.mul(F.sub(x, y), z) === F.add(a, b)), (x, y, z, a, b) => ForeignField.assertMul(ForeignField.Sum(x).sub(y), z, ForeignField.Sum(a).add(b), F.modulus), 'assertMul negative');
    ...
    equivalentProvable({ from: [big264], to: f })(F.mod, (x) => ForeignField.toCanonical(x, F.modulus), 'to canonical');
    // addition can fail on two unreduced inputs because we can get x + y - f > 2^264
    equivalentProvable({ from: [big264, f], to: big264 })(F.add, (x, y) => ForeignField.add(x, y, F.modulus), 'add unreduced');
    // subtraction doesn't work with unreduced y because the range check on the result prevents x-y < -f
    equivalentProvable({ from: [big264, f], to: big264 })(F.sub, (x, y) => ForeignField.sub(x, y, F.modulus), 'sub unreduced');
    equivalentProvable({ from: [big258, big258], to: f })(F.mul, (x, y) => ForeignField.mul(x, y, F.modulus), 'mul unreduced');
    equivalentProvable({ from: [big258], to: f })((x) => F.inverse(x) ?? throwError('no inverse'), (x) => ForeignField.inv(x, F.modulus), 'inv unreduced');
    ...
    equivalentProvable({ from: [big258, big258], to: f })((x, y) => F.div(x, y) ?? throwError('no inverse'), (x, y) => ForeignField.mul(x, ForeignField.inv(y, F.modulus), F.modulus), 'div unreduced');
    equivalent({ from: [big264], to: unit })((x) => assertWeakBound(x, F.modulus), (x) => ForeignField.assertAlmostReduced([x], F.modulus));
    equivalentProvable({ from: [big264, big264], to: unit })((x, y) => assert(x < y, 'not less than'), (x, y) => ForeignField.assertLessThan(x, y));
    equivalentProvable({ from: [big264, big264], to: unit })((x, y) => assert(x <= y, 'not less than or equal'), (x, y) => ForeignField.assertLessThanOrEqual(x, y));
    // sumchain of 5
    equivalentProvable({ from: [array(f, 5), array(sign, 4)], to: f })((xs, signs) => sum(xs, signs, F), (xs, signs) => ForeignField.sum(xs, signs, F.modulus), 'sumchain 5');
    // sumchain up to 100
    let operands = array(record({ x: f, sign }), Random.nat(100));
    equivalentProvable({ from: [f, operands], to: f })((x0, ts) => {
...
let mrc = ['RangeCheck0', 'RangeCheck0', 'RangeCheck1', 'Zero'];
constraintSystem.fromZkProgram(ffProgram, 'sumchain', ifNotAllConstant(and(contains([addChain(chainLength), mrc]), withoutGenerics(equals([...addChain(chainLength), ...mrc])))));
let mulChain = ['ForeignFieldMul', 'Zero'];
let mulLayout = ifNotAllConstant(and(contains([mulChain, mrc, mrc, mrc]), withoutGenerics(equals([...mulChain, ...repeat(3, mrc)]))));
let invLayout = ifNotAllConstant(and(contains([mrc, mulChain, mrc, mrc, mrc]), withoutGenerics(equals([...mrc, ...mulChain, ...repeat(3, mrc)]))));
...
await equivalentAsync({ from: [array(f, chainLength)], to: f }, { runs })((xs) => sum(xs, signs, F), async (xs) => {
    let { proof } = await ffProgram.sumchain(xs);
    assert(await ffProgram.verify(proof), 'verifies');
    return proof.publicOutput;
}, 'prove chain');
await equivalentAsync({ from: [big264, big264], to: f }, { runs })((x, y) => {
...
await equivalentAsync({ from: [f, f], to: f }, { runs })((x, y) => F.div(x, y) ?? throwError('no inverse'), async (x, y) => {
    let { proof } = await ffProgram.div(x, y);
    assert(await ffProgram.verify(proof), 'verifies');
    return proof.publicOutput;
}, 'prove div');
await equivalentAsync({ from: [f, f], to: unit }, { runs })((x, y) => assert(x < y, 'not less than'), async (x, y) => {
    let { proof } = await ffProgram.assertLessThan(x, y);
    assert(await ffProgram.verify(proof), 'verifies');
}, 'prove less than');
// assert mul example
// (x - y) * (x + y) = x^2 - y^2
function assertMulExample(x, y, f) {
    // witness x^2, y^2
...
function assertMulExampleNaive(x, y, f) {
    // witness x^2, y^2
    let x2 = Provable.witness(Field3, () => ForeignField.mul(x, x, f));
    let y2 = Provable.witness(Field3, () => ForeignField.mul(y, y, f));
    // assert (x - y) * (x + y) = x^2 - y^2
...
let addChainedIntoMul = ['ForeignFieldAdd', ...mulChain];
constraintSystem('assert mul', from2, (x, y) => assertMulExample(x, y, F.modulus), or(and(contains([addChain(2), addChain(2), addChainedIntoMul]), 
// assertMul() doesn't use any range checks besides on internal values and the quotient
containsNTimes(2, mrc)), allConstant));
// helper
function containsNTimes(n, pattern) {
    return and(contains(repeat(n, pattern)), not(contains(repeat(n + 1, pattern))));
}
function sum(xs, signs, F) {
    let sum = xs[0];
...
function assertWeakBound(x, f) {
    assert(x >= 0n && x >> l2 <= f >> l2, 'weak bound');
}
//# sourceMappingURL=foreign-field-gadgets.unit-test.js.map