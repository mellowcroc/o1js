...
describe('Provable', () => {
    ...
    it('Provable.switch throws when mask has >1 true elements', () => {
        expect(() => Provable.switch([Bool(true), Bool(true), Bool(false)], Int64, [
            Int64.from(-1),
            Int64.from(-2),
            Int64.from(-3),
    ...
    it('Provable.assertEqual', async () => {
        ...
        await Provable.runAndCheck(() => {
            ...
            expect(() => Provable.assertEqual(FieldAndBool, { x, b }, { x: Field(5), b: Bool(true) })).toThrow();
            expect(() => Provable.assertEqual(b, PrivateKey.random())).toThrow('must contain the same number of field elements');
        });
    });
    it('Provable.equal', async () => {
        ...
        function expectBoolean(b, expected) {
            Provable.asProver(() => {
...
//# sourceMappingURL=provable.test.js.map