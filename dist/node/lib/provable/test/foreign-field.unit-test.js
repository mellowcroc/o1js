...
import { bool, equivalentProvable as equivalent, field, first, spec, throwError, unit, } from '../../testing/equivalent.js';
...
equivalent({ from: [f], to: f })((x) => Fq.inverse(x) ?? throwError('division by 0'), (x) => x.inv());
equivalent({ from: [f, f], to: f })((x, y) => Fq.div(x, y) ?? throwError('division by 0'), (x, y) => x.div(y));
// equality with a constant
equivalent({ from: [f, first(f)], to: bool })((x, y) => x === y, (x, y) => x.equals(y));
equivalent({ from: [f, f], to: unit })((x, y) => x === y || throwError('not equal'), (x, y) => x.assertEquals(y));
equivalent({ from: [f, first(u264)], to: unit })((x, y) => x < y || throwError('not less than'), (x, y) => x.assertLessThan(y));
// toBits / fromBits
equivalent({ from: [f], to: f })((x) => x, (x) => {
    let bits = x.toBits();
...
equivalent({
    from: [field],
    to: f,
})((x) => x, (field) => ForeignScalar.Unsafe.fromField(field));
console.log('✓ All Field conversion tests passed!');
//# sourceMappingURL=foreign-field.unit-test.js.map