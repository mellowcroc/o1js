...
import { equivalentProvable as equivalent, equivalentAsync, field, fieldWithRng, record, } from '../../testing/equivalent.js';
...
const divMod32Helper = (x) => {
    let quotient = x >> 32n;
    let remainder = x - (quotient << 32n);
    return { remainder, quotient };
};
const divMod64Helper = (x) => {
    let quotient = x >> 64n;
...
await equivalentAsync({ from: [field], to: divModOutput }, { runs: 3 })((x) => {
    assert(x < 1n << 64n, `x needs to fit in 64bit, but got ${x}`);
    return divMod32Helper(x);
}, async (x) => {
...
//# sourceMappingURL=arithmetic.unit-test.js.map