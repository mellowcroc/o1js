...
test(Random.publicKey, randomTokenId, (publicKey, field) => {
    ...
    let ml = Field(mlTest.tokenId.derive(Ml.fromPublicKey(tokenOwner), Ml.constFromField(parentTokenId)));
    expect(js).toEqual(ml);
});
// - provable
test(Random.publicKey, randomTokenId, (publicKey, field) => {
    ...
    runAndCheckSync(() => {
        ...
        let js = TokenId.derive(tokenOwner, parentTokenId);
        let ml = Field(mlTest.tokenId.deriveChecked(Ml.fromPublicKeyVar(tokenOwner), Ml.varFromField(parentTokenId)));
