...
const ZkappCommand = {
    ...
    parse(json) {
        ...
        try {
            ...
            throw new Error(`Failed to parse ZkappCommand from JSON string: ${error instanceof Error ? error.message : 'Invalid JSON'}`);
        }
        try {
            return this.fromJSON(parsedJson);
        }
        catch (error) {
            throw new Error(`Failed to construct ZkappCommand from parsed JSON: ${error instanceof Error ? error.message : 'Invalid ZkappCommand structure'}`);
        }
    },
    fromJSON(json) {
