...
import { AccountUpdate, AccountUpdateForest, Mina, Permissions, PrivateKey, TokenContract, UInt64, method, } from 'o1js';
/**
...
], BasicTokenContract.prototype, "approveBase", null);
/**
...
__decorate([
    ...
    __metadata("design:paramtypes", [AccountUpdateForest]),
    __metadata("design:returntype", Promise)
], CustomPermissionsTokenContract.prototype, "approveBase", null);
/**
...
__decorate([
    ...
    __metadata("design:paramtypes", [AccountUpdateForest]),
    __metadata("design:returntype", Promise)
], StrictAccessTokenContract.prototype, "approveBase", null);
/**
...
__decorate([
    ...
    __metadata("design:paramtypes", [AccountUpdateForest]),
    __metadata("design:returntype", Promise)
], ImpossibleAccessTokenContract.prototype, "approveBase", null);
...
// Test 1: Basic deploy sets access to proofOrSignature
{
    ...
    assert(!access.constant.toBoolean() &&
        !access.signatureNecessary.toBoolean() &&
        access.signatureSufficient.toBoolean(), 'basic deploy should set access to proofOrSignature');
...
// Test 2: Custom permissions set in init() are preserved after deploy
{
    ...
    assert(!perms.access.constant.toBoolean() &&
        !perms.access.signatureNecessary.toBoolean() &&
        perms.access.signatureSufficient.toBoolean(), 'access should be proofOrSignature');
    ...
    assert(perms.setPermissions.constant.toBoolean() &&
        perms.setPermissions.signatureNecessary.toBoolean() &&
        !perms.setPermissions.signatureSufficient.toBoolean(), 'setPermissions should be impossible (preserved from init)');
    ...
    assert(!perms.send.constant.toBoolean() &&
        !perms.send.signatureNecessary.toBoolean() &&
        !perms.send.signatureSufficient.toBoolean(), 'send should be proof (preserved from init)');
...
// Test 3: Stronger access permission (proof) set in init() is preserved
{
    ...
    assert(!access.constant.toBoolean() &&
        !access.signatureNecessary.toBoolean() &&
        !access.signatureSufficient.toBoolean(), 'access should remain proof() when set in init (not downgraded to proofOrSignature)');
...
// This test confirms deploy() doesn't silently downgrade impossible() to proofOrSignature().
{
    ...
    assert(capturedAccess.constant &&
        capturedAccess.signatureNecessary &&
        !capturedAccess.signatureSufficient, 'access should remain impossible() in account update (not downgraded to proofOrSignature)');
...
// Test 5: Unauthorized token operations are still rejected
{
    ...
    await assert.rejects(() => mintTx.sign([sender.key]).send(), /Update_not_permitted_access/, 'unauthorized mint should be rejected');
    console.log('✓ unauthorized token operations are still rejected');
}
console.log('\nAll TokenContract deploy permission tests passed.');
//# sourceMappingURL=token-contract-deploy.unit-test.js.map