...
import { UInt64, UInt32, SmartContract, Mina, AccountUpdate, method, PublicKey, Bool, Field, } from 'o1js';
    ...
    it('unsatisfied requireEquals should be rejected (numbers)', async () => {
        for (let precondition of implementedNumber) {
            await expect(async () => {
                let tx = await Mina.transaction(feePayer, async () => {
                    let p = precondition().get();
                    precondition().requireEquals(p.add(1));
                    AccountUpdate.attachToTransaction(contract.self);
                });
...
//# sourceMappingURL=precondition.test.js.map