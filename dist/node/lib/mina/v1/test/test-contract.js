...
async function testLocal(Contract, { proofsEnabled, offchainState, batchReducer, autoDeploy = true, }, callback) {
    // instance-independent setup: compile programs
    batchReducer?.setContractClass(Contract);
    if (proofsEnabled) {
    ...
    async function execute(Local) {
        ...
        let i = 2;
        let accounts = new Proxy(originalAccounts, {
            get(accounts, name) {
                if (name in accounts)
        ...
        let newAccounts = new Proxy({}, {
            get(accounts, name) {
                if (name in accounts)
                    return newAccounts[name];
    ...
    if (proofsEnabled === 'both' || proofsEnabled === false) {
        if (proofsEnabled === 'both')
            console.log('(without proofs)');
        await execute(Local);
    }
...
async function runInstruction(spec, instruction) {
...
function expectState(state, expected, message) {
    let trace = Error().stack?.slice(5);
    return { type: 'expect-state', state, expected, trace, label: message };
}
function expectBalance(address, expected, message) {
    let trace = Error().stack?.slice(5);
    return {
        type: 'expect-balance',
...
// error helper
async function assertionWithTrace(trace, fn) {
    try {
        await fn();
    }
    catch (err) {
...
async function assertionWithTracePreferInner(trace, fn) {
    try {
        await fn();
    }
    catch (err) {
