...
function createAccountUpdateWithAuth(authKind) {
...
}
{
    ...
    expect(() => verifyTransactionLimits({ accountUpdates })).not.toThrow();
}
{
    const accountUpdates = Array.from({ length: 16 }, () => createAccountUpdateWithAuth('proof'));
    expect(() => verifyTransactionLimits({ accountUpdates })).toThrow(/too many segments/);
}
{
...
{
    const accountUpdates = [createAccountUpdateWithAuth('signed')];
    for (let i = 0; i < 7; i++) {
        accountUpdates.push(createAccountUpdateWithAuth('proof'));
        accountUpdates.push(createAccountUpdateWithAuth('signed'));
    }
...
//# sourceMappingURL=mina.unit-test.js.map