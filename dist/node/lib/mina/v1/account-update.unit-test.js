...
function createAccountUpdateWithMayUseToken(mayUseToken) {
    let accountUpdate = AccountUpdate.default(address);
...
{
    let accountUpdate = createAccountUpdate();
    expect(accountUpdate.body.preconditions.account.actionState.value.toString()).toEqual('25079927036070901246064867767436987657692091363973573142121686150614948079097');
}
// creates the right empty vk hash
{
    let accountUpdate = createAccountUpdate();
    expect(accountUpdate.body.authorizationKind.verificationKeyHash.toString()).toEqual(mocks.dummyVerificationKeyHash);
}
// does not throw an error if private key is missing unless if .send is executed
{
    ...
    tx.sign([]);
    await expect(tx.send()).rejects.toThrow('Check signature: Invalid signature on fee payer for key');
}
