...
import { State, state, UInt64, Bool, SmartContract, Mina, AccountUpdate, method, PublicKey, Permissions, VerificationKey, Int64, TokenId, TokenContract as TokenContractBase, } from 'o1js';
...
class TokenContract extends TokenContractBase {
     ...
     */
    async deploy_(address, verificationKey) {
    ...
    async mint(receiverAddress, amount) {
        let totalAmountInCirculation = this.totalAmountInCirculation.get();
        this.totalAmountInCirculation.requireEquals(totalAmountInCirculation);
        let newTotalAmountInCirculation = totalAmountInCirculation.add(amount);
        newTotalAmountInCirculation.value.assertLessThanOrEqual(this.SUPPLY.value, "Can't mint more than the total supply");
        this.internal.mint({ address: receiverAddress, amount });
        this.totalAmountInCirculation.set(newTotalAmountInCirculation);
    }
    async burn(receiverAddress, amount) {
    ...
    async approveTransfer(senderAddress, receiverAddress, amount, senderAccountUpdate) {
...
], TokenContract.prototype, "deploy_", null);
__decorate([
    method,
    __metadata("design:type", Function),
...
__decorate([
    ...
    __metadata("design:paramtypes", [PublicKey,
        ...
        AccountUpdate]),
    __metadata("design:returntype", Promise)
], TokenContract.prototype, "approveTransfer", null);
class B extends SmartContract {
    async approveSend(amount) {
...
class C extends SmartContract {
    ...
    async approveIncorrectLayout(amount) {
        this.balance.subInPlace(amount);
...
__decorate([
    ...
    __metadata("design:paramtypes", [UInt64]),
    __metadata("design:returntype", Promise)
], C.prototype, "approveIncorrectLayout", null);
let feePayer;
let tokenAccount;
let token;
let tokenId;
        ...
        */
        describe('Token Contract Creation/Deployment', () => {
            ...
            test('setting a valid token symbol on a token contract', async () => {
                await (await Mina.transaction({ sender: feePayer }, async () => {
        ...
        describe('Mint token', () => {
            ...
            test('token contract can successfully mint and updates the balances in the ledger (signature)', async () => {
                await (await Mina.transaction({ sender: feePayer }, async () => {
                    ...
                    .sign([feePayer.key, tokenAccount.key])
                    .send();
                expect(Mina.getBalance(bAccount, tokenId).value.toBigInt()).toEqual(100000n);
            });
        ...
        */
        describe('Burn token', () => {
            ...
            test('token contract can successfully burn and updates the balances in the ledger (signature)', async () => {
                await (await Mina.transaction(feePayer, async () => {
                    AccountUpdate.fundNewAccount(feePayer);
                    await token.mint(bAccount, UInt64.from(100_000));
                    token.requireSignature();
                }))
                ...
                await (await Mina.transaction(feePayer, async () => {
                    ...
                    .send();
                expect(Mina.getBalance(bAccount, tokenId).value.toBigInt()).toEqual(90000n);
            });
        ...
        */
        describe('Transfer', () => {
            ...
            test('change the balance of a token account after sending', async () => {
                ...
                await tx.send();
                expect(Mina.getBalance(bAccount, tokenId).value.toBigInt()).toEqual(90000n);
                expect(Mina.getBalance(cAccount, tokenId).value.toBigInt()).toEqual(10000n);
            });
        ...
        describe('Transfer', () => {
            ...
            test('should fail to approve with an incorrect layout', async () => {
                await (await Mina.transaction(feePayer, async () => {
                    ...
                    .sign([feePayer.key, tokenAccount.key])
                    .send();
                await expect(() => Mina.transaction(feePayer, async () => {
