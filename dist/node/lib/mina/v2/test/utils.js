...
export function stripPrototypes(x) {
    if (typeof x === 'object') {
...
export function v1FetchLayout(root, path) {
    let here = root;
    for (const key of path) {
        switch (here.type) {
            ...
            case 'option':
                if (key !== 'inner')
                    throw new Error(`invalid layout path: found ${here.type}, and expected the key to be inner, but path had a key of ${key}`);
                here = here.inner;
                break;
            default:
...
export function testV2Encoding(Type, value) {
...
export function testV1V2ClassEquivalence(V1Type, V2Type, v2InternalReprArg) {
...
export function testV1V2ValueEquivalence(V1Type, V2Type, v1Value, v2Value, v2InternalReprArg) {
    ...
    // conversion
    expect(stripPrototypes(v2Value.toInternalRepr(v2InternalReprArg))).toEqual(stripPrototypes(v1Value));
    ...
    // v1 -> v2 via fields
    expect(V1Type.fromFields(V2Type.toFields(v2Value), V2Type.toAuxiliary(v2Value, v2InternalReprArg))).toEqual(v1Value);
    // input equality
