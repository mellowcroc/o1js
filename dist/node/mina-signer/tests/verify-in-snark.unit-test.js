...
const MyProgram = ZkProgram({
    ...
    methods: {
        verifySignature: {
            ...
            async method(signature, message) {
...
await expect(() => MyProgram.verifySignature(invalidSignature, fieldsSnarky)).rejects.toThrow('Constraint unsatisfied');
...
// can't verify in snark
await expect(() => MyProgram.verifySignature(signature, wrongFields)).rejects.toThrow('Constraint unsatisfied');
//# sourceMappingURL=verify-in-snark.unit-test.js.map