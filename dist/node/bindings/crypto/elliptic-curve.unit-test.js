...
for (let [G, Field, Scalar] of [
    ...
    test(randomPoint, randomPoint, randomPoint, randomScalar, randomScalar, randomField, (X, Y, Z, x, y, f) => {
        ...
        assert(G.equal(X, G.zero) ||
            Field.square(ya) === Field.add(Field.add(Field.power(xa, 3n), Field.mul(G.a, xa)), G.b), 'affine on curve (or zero)');
    });
}
// helper
function curveWithFields(params) {
    ...
    return [Projective, Affine.Field, Affine.Scalar];
}
