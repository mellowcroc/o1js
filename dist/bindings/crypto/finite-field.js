...
export { createField, Fp, Fq, p, q, mod, inverse };
// CONSTANTS
...
function fastInverse(x, p, n, kmax, twoToMinusKmax) {
    x = mod(x, p);
    if (x === 0n)
        return undefined;
    // fixed constants
...
function randomField(p, sizeInBytes, hiBitMask) {
...
function createField(p, constants) {
    ...
    const exportedInverse = twoToMinusKmax !== undefined
        ? (x) => fastInverse(x, p, n, kmax, twoToMinusKmax)
        : (x) => inverse(x, p);
    return {
        modulus: p,
        sizeInBits,
 ...
 */
function computeFieldConstants(p) {
...
//# sourceMappingURL=finite-field.js.map